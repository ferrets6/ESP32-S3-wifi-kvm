...
extern "C" uint16_t tusb_hid_load_descriptor(uint8_t *dst, uint8_t *itf) {
  ...
  // Single IN-only endpoint, 8-byte reports, 10ms polling: matches two
  // independently-confirmed-working boot keyboards (a real Logitech K120
  // and a from-scratch CC2544 gadget) byte-for-byte, instead of this
  // core's normal 2-endpoint/64-byte/1ms defaults.
  ...
  uint8_t descriptor[TUD_HID_DESC_LEN] = {
    TUD_HID_DESCRIPTOR(*itf, str_index, tinyusb_interface_protocol, tinyusb_hid_device_descriptor_len, (uint8_t)(0x80 | ep_in), 8, 10)
  };
  *itf += 1;
...
#ifdef KVM_KEYBOARD_ONLY
    tinyusb_enable_interface2(USB_INTERFACE_HID, TUD_HID_DESC_LEN, tusb_hid_load_descriptor, itf_protocol == HID_ITF_PROTOCOL_KEYBOARD);
#else
